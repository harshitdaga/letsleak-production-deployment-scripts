    ...
    if (isValidAccountRequest()) {
        ...
        if(result.isSuccess) {
            ...
            try{
                ...
                var loginResult = loginWrapper._doLogin({username:username},"CreateAccount Autologin");
                ...
                log.debug("/createAccount autoloin success",{loginResult: loginResult});
            }catch (e){
                log.error("/createAccount Autologin failed", {request:oriRequestData.username,e: e.stack});
            }

            //Droping WELCOME inbox
            var inboxNotif = new AppClass.Notification.Inbox();
            var requestObj = {
                userId: username,
                postId: "",
                type: inboxNotif.types.WELCOME,
                paramObj: {}
            };
            inboxNotif.process(requestObj);
        }
        return result;
    }
};
